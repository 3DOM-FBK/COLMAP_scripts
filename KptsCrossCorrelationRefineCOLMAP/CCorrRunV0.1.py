...
def main():
    ...
    # Storing all the images in a dictionary
    start = time.time()
    img_dict = {}
    img_list = os.listdir(images_path)
    for i,image in enumerate(img_list):
        image_name = Path(image)
        img = cv2.imread(r"{}/{}".format(args.Images, image_name), 0)
        img_dict[image] = img
        print("Processed img {}/{}".format(i, len(img_list)-1))
    end = time.time()
    print("Storing all the images in a dictionary:", end-start)
    ...
    for track_key in track_dic:
        ...
        for current_ktp in current_track:
            ...
            if filled_template == False:
                img = img_dict[image_name]
                img_width = img.shape[1]
                img_height = img.shape[0]
                if (x-patch_width//2) < 0 or (y-patch_hight//2) < 0 or (x+patch_width//2+1) > img_width or (y+patch_hight//2+1) > img_height:
                    break
                else:
                    template = img[y-patch_hight//2 : y+patch_hight//2+1, x-patch_width//2 : x+patch_width//2+1]
                    if silent == 'False':
                        patchNP = Image.fromarray(template)
                        patchNP.show()
                ...
                img_width = img.shape[1]
                img_height = img.shape[0]
                if x-patch_width//2*A < 0 or y-patch_hight//2*A < 0 or x+patch_width//2*A+1 > img_width or y+patch_hight//2*A+1 > img_height:
                    break
                else:
                    ref_patch = img[y-patch_hight//2*A : y+patch_hight//2*A+1, x-patch_width//2*A : x+patch_width//2*A+1]
                    if silent == 'False':
                        patchNP = Image.fromarray(ref_patch)
                        patchNP.show()
