...
def main():
    ...
    print("N of 3D points: {}".format(len(track_dic.keys())))
    ...
    for pairID in pairID_and_pairs.keys():
        ...
        kpt1 = np.asarray(image_id_and_keypoints[i1]).reshape(-1,6)
        kpt2 = np.asarray(image_id_and_keypoints[i2]).reshape(-1,6)
    ...
    for key in image_id_and_keypoints:
        ...
        refined_keypoints[imgID_name[key]] = kp_matrix
    ...
    for track_key in track_dic:
        ...
        for t in range(int(len(current_track_list)/2)):
            ...
            kpt = np.asarray(image_id_and_keypoints[im_id]).reshape(-1,6)
            #print(kpt[0:5,:])
            ...
            if filled_template == False:
                ...
                if x-patch_width//2*A < 0 or y-patch_hight//2*A < 0 or x+patch_width//2*A+1 > img.width or y+patch_hight//2*A+1 > img.height:
                    ...
                    if mode == 'SSD':
                        ...
                        print("ref_patch.shape, template.shape", ref_patch.shape, template.shape)
        ...
        print("Processed track {}".format(c))
    ...
    for key_im_name in refined_keypoints:
        for value in imgID_name:
            if imgID_name[value] == key_im_name:
                ...
                print(refined_keypoints[key_im_name])
